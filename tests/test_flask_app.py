...
class FlaskAppTestCase(unittest.TestCase):
    ...
    def test_valid_request(self):
        response = self.app.get('/api/gita?reference=1&author_id=16', headers=self.headers)
        self.assertEqual(response.status_code, 200)
        self.assertIn('content', response.get_json())

    def test_invalid_request_no_reference(self):
        response = self.app.get('/api/gita?author_id=16', headers=self.headers)
        self.assertEqual(response.status_code, 400)
        self.assertIn('error', response.get_json())

    def test_invalid_request_bad_reference(self):
        response = self.app.get('/api/gita?reference=invalid&author_id=16', headers=self.headers)
        self.assertEqual(response.status_code, 400)
        self.assertIn('error', response.get_json())
